...
class BookmarkAdapter(private var bookmarkData: List<BookmarkResult>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    fun bookmarkUpdateData(newBookmarkData: List<BookmarkResult>) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder = //RecyclerView. 지워도 될듯
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        val bookmarkResult = bookmarkData[position]
        holder.binding.bookmarkWordData.text = bookmarkResult.word
        holder.binding.boomkarkMean1.text = bookmarkResult.word
        holder.binding.boomkarkMean2.text = bookmarkResult.word
        holder.binding.boomkarkMean3.text = bookmarkResult.word

        if (bookmarkResult.isSelect) {
            holder.binding.bookmarkWordData.setTextColor(ContextCompat.getColor(holder.itemView.context, R.color.rightgreen))
        } else {
            holder.binding.bookmarkWordData.setTextColor(ContextCompat.getColor(holder.itemView.context, R.color.wrongred))
        }
